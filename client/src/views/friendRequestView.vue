<script setup>
    ...
    const confirmRequest = async (friendUsername) => {
        const response = await fetch(`http://localhost:8080/user/confirmRequest?username=${username.value}&friendUsername=${friendUsername}`, {
            method: 'POST',
        });
    ...
    const rejectRequest = async (friendUsername) => {
        const response = await fetch(`http://localhost:8080/user/rejectRequest?username=${username.value}&friendUsername=${friendUsername}`, {
            method: 'POST',
        });
    ...
    const revokeRequest = async (friendUsername) => {
        const response = await fetch(`http://localhost:8080/user/rejectRequest?username=${friendUsername}&friendUsername=${username.value}`, {
            method: 'POST',
        });
    ...
    const fetchIncomingFriendRequests = async () => {
            const route = "http://localhost:8080/user/getIncomingFriendRequests?username=" + username.value;
    ...
    const fetchOutgoingFriendRequests = async () => {
            const route = "http://localhost:8080/user/getOutgoingFriendRequests?username=" + username.value;
            const response = await fetch(route, {
                method: "GET",
                headers: {
                    "Access-Control-Allow-Origin": "*",
